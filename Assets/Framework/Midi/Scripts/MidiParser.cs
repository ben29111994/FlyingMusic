...
public class MidiParser : MonoBehaviour {
    private const float MILISECONDS_PER_SECOND = 1000000f;
    /// <summary>
    ...
    public static int ParseNotesData(byte[] midiData, ref MidiData levelData, List<int> tracksToParse = null) {
        ...
        float secondsPerPulse = (float)songFile.MicroSecondsPerQuarterNote / MILISECONDS_PER_SECOND / (float)songFile.MidiHeader.deltaTicksPerQuarterNote;

        //start parsing from track 1, since track 0 is playback events
            ...
            for (int j = 0; j < midiEvents.Length; j++) {
                if (midiEvents[j].isChannelEvent()) {
                    ...
                    if (evt.midiChannelEvent == MidiHelper.MidiChannelEvent.Note_On) {
                        ...
                        // this is MIDI file's specification to calculate volume
